...
def launch():
	...
	cmds = ['python hokanjo/nodeserver.py -id %d -f data/%d.tch -cfg test/config.yaml &> logs/%d.log' % (node.id, node.id, node.id) for node in cfg.active_deployment.nodes.itervalues()]
