...
def launch():
	...
	cmds = ['python hokanjo/nodeserver.py -id %s -f var/data/%s.tch -cfg test/config.yaml &> var/log/%s.log' % (node.id, node.id, node.id) for node in cfg.active_deployment.nodes.itervalues()]
